...
temp_retSeries = price2ret(data(1:51,:)');
retSeries = temp_retSeries(:,2:51);
%下面计算这51只股票之间的协方差矩阵以及它们各自的期望收益，但是注意，这是要以五年为期限的。也就是说，一共应该有10个协方差矩阵和10个期望收益。
Covariance = zeros(10,n(1)-2,n(1)-2);
meanRet = zeros(10,n(1)-2);
for i=1:10
    s = sdate(i);
    t = edate(i);
...
Aeq = zeros(10,2,50);
for i=1:10
    Aeq(i,:,:) = [meanRet(i,:);ones(1,50)];
end
beq = [0.1,1];
weight = zeros(10,50);
for i=1:10
    now_H = reshape(H(i,:,:),50,50);
    now_Aeq = reshape(Aeq(i,:,:),2,50);
...
curweight = zeros(n(2)-2-edate(1),50);
...
for day=2:n(2)-2-edate(1)
    HS300(day) = HS300(day-1)*exp(temp_retSeries(day+edate(1)-1,1));
end
plot(HS300,'r');
hold on
plot(netvalue,'b');
